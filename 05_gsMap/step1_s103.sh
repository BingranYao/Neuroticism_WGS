...
mkdir -p "${OUTROOT}/logs/s103" "${OUTROOT}/nohup_logs"

mapfile -t H5ADS < <(find "${INDIR}" -maxdepth 1 -type f -name "*_s103_rot.h5ad" | sort)
(( ${#H5ADS[@]} > 0 )) || { echo "[ERROR] no s103 h5ad files found under ${INDIR}" >&2; exit 1; }
...
for h5ad in "${H5ADS[@]}"; do
  ...
  log_file="${OUTROOT}/logs/s103/${sample}.step1_3.log"
...
(( fail == 0 )) || exit 1
echo "[DONE] all s103 samples finished"
